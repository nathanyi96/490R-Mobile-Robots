...
class PIDController(BaseController):
    ...
    def get_reference_index(self, pose):
        ...
        with self.path_lock:
            ...
            return np.argmin(dist)#+5)%self.path.shape[0]  # or +1 to make sure ref is ahead of current pos
    ...
    def reset_params(self):
        ...
        '''
        with self.path_lock:
            self.kp = float(rospy.get_param("/pid/kp", 3.0)) # 0.65
            self.kd = float(rospy.get_param("/pid/kd", 2.0)) # 0.8
