...
def saw():
    """
    Generates the sawtooth path. Increase alpha to generate higher frequency signal
    """
...
def circle():
    """
    Generate a circle. Change radius to get a bigger circle
    """
    waypoint_sep = 0.1
    radius = 2.5
    center = [0, radius]
...
def left_turn():
    """
    Generate a straight line and left-turn (90-degrees counterclockwise) path.
    Tune turn_radius and straight_len.
    """
    waypoint_sep = 0.1
    turn_radius = 1.5
    straight_len = 10.0
...
def cse022_path_up():
    with open('cse022_path_up.pickle', 'r') as f:
        p = pickle.load(f)
    return p


plans = {'circle': circle, 'left turn': left_turn, 'right turn': right_turn, 'saw': saw, 'cse022 real path': cse022_path,
'cse022 up' : cse022_path_up}
plan_names = ['circle', 'left turn', 'right turn', 'saw', 'cse022 real path', 'cse022 up']
...
if __name__ == '__main__':
    ...
    # planner = raw_input("Run with planner? (y,[n]) ")
    # if 'y' in planner:
    #     configs = run_planner()
    # else:
