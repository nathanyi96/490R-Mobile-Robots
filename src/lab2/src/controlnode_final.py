...
from lab2.srv import FollowPath, FollowPath_log, StampedFollowPath, SignalPathComplete, SignalPathCompleteRequest
...
class ControlNode:
    ...
    def start(self, name):
        ...
        while not rospy.is_shutdown():
            ...
            if ip is not None and self.controller.ready():
                ...
                if pose is None or index is None or ip is None:
                    continue
                ...
                if self.controller.path_complete(ip, error):
                    # self.log_error()
                    rospy.logdebug('type {}'.format(self.current_path_type))
                    self.path_event.clear()
                    self.notifyComplete(self.current_path_type)
    ...
    def setup_pub_sub(self):
        ...
        rospy.Service("/controller/follow_path", StampedFollowPath, self.cb_path)
        ...
        self.notifyComplete = rospy.ServiceProxy("final_planner/path_complete", SignalPathComplete)
        rospy.sleep(0.5)
    ...
    def cb_path(self, msg):
        ...
        self.current_path_type = msg.type
        self.controller.set_path(path)
        if self.current_path_type == 'main_again':
            self.current_path_type = 'main'
            self.controller.prev_idx = 10
        self.path_event.set()
        print "Path set"
        return True
